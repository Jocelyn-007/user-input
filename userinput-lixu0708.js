// ======================================================
// Pacita Abad 'Wheels of Fortune' — User Input Version
// Base modules: PaletteSystem / DotSystem / WheelSystem / LayoutSystem
// New behaviour for individual task:
// - Mouse position and mouse press animate the wheels (scale + rotation).
// - Key 'A' toggles bead arcs.
// - 'R' regenerates layout (Shift+R keeps seed), 'S' saves a PNG.
// ======================================================
...
let beadArcs = [];
let backgroundDots = [];
let showArcs = true; // user can toggle bead arcs with the 'A' key
...
function setup() {
  ...
  noStroke();
  regenerate(false);
  // We keep the draw loop running so that user input can drive animation.
}

function draw() {
  // Map mouse X to a subtle background hue shift so the canvas feels responsive.
  ...
  // Optional bead arcs, toggled by the 'A' key
  if (showArcs) {
  ...
  // Wheels: first update animation state from user input, then draw
...
function keyPressed() {
  ...
  if (key === 'S' || key === 's') {
    saveCanvas('wheels_of_fortune_user_input', 'png');
  }
  // Toggle bead arcs visibility
  if (key === 'A' || key === 'a') {
    showArcs = !showArcs;
  }
}
